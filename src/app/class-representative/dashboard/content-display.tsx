    ...
    <div>
      ...
      <div className="relative">
        <div className="horizontal-scrollbar flex w-full space-x-6 pb-4">
          {items.map((item) => (
            <Card key={item.id} className="min-w-[300px] max-w-[300px] flex flex-col group animated-gradient-border transform transition-all duration-300 hover:-translate-y-2">
              <CardHeader>
  ...
  if (!contents || contents.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-muted-foreground/30 py-24 text-center">
        ...
        <p className="text-muted-foreground mt-2">Click the "Add Content" button to share your knowledge!</p>
      </div>
    );
  }
