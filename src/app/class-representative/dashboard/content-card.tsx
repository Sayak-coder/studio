  ...
  return (
    ...
    >
      ...
      <div className="relative h-[160px] w-full flex-shrink-0">
             ...
             {isEditable && onEdit && onDelete && (
              <>
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button variant="secondary" size="icon" className="h-8 w-8 rounded-full" onClick={() => onEdit(item)}>
                ...
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                       <Button variant="destructive" size="icon" className="h-8 w-8 rounded-full" onClick={() => onDelete(item.id)}>
         ...
         {item.fileUrl ? (
             <a href={item.fileUrl} target="_blank" rel="noopener noreferrer" className="block h-full w-full">
      ...
      <div className="relative flex flex-grow flex-col p-4">
        ...
        >
         ...
         <p className="text-xs text-muted-foreground/80 mt-2 pt-2 border-t border-muted-foreground/20">By: {item.authorName}</p>
      </div>
    </div>
  );
};
