...
const RECENT_SEARCHES_KEY = 'edubot_cr_recent_searches';

export const useCRSearch = (onSearchChange: (results: Content[] | null) => void, allContent: Content[]) => {
  ...
  const filteredResults = useMemo(() => {
    ...
    return allContent.filter(
        (item) =>
          ...
          item.authorName.toLowerCase().includes(lowerCaseQuery)
      );
